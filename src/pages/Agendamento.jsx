  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      setForm({
        ...
        dia: '', hora: '', exame: '', valor: '', pagamento: 'Pix', telefone: '', endereco: '', observacoes: ''
      });
      carregarDados();
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 md:p-8 rounded-xl shadow-md border border-slate-200 mb-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="md:col-span-3">
            ...
            <input type="text" placeholder="Observação para essa coleta" className="w-full border-2 rounded-lg p-3 outline-none focus:border-[#006363]" value={form.observacoes} onChange={e => setForm({ ...form, observacoes: e.target.value })} />
          </div>
                ...
                {editandoId === item.id ? (
                  ...
                  <>
                    <td className="p-4 md:p-5 align-top md:border-r border-slate-100 space-y-2 bg-blue-50/50 block md:table-cell">
                      ...
                      <label className="text-[9px] font-bold text-blue-400 uppercase">Equipe</label>
                      <select className="w-full border p-1 text-xs rounded mb-1" value={itemEditado.motorista} onChange={e => setItemEditado({...itemEditado, motorista: e.target.value})}>
                        {motoristas.map((m, i) => <option key={i} value={m.nome}>{m.nome}</option>)}
                      </select>
                    ...
                    <td className="p-4 md:p-5 align-top space-y-2 bg-blue-50/50 block md:table-cell">
                      <label className="text-[9px] font-bold text-blue-400 uppercase">Paciente / Telefone</label>
                      ...
                      <label className="text-[9px] font-bold text-blue-400 uppercase">Observações</label>
                      <input type="text" className="w-full border p-2 rounded text-sm" value={itemEditado.observacoes} onChange={e => setItemEditado({...itemEditado, observacoes: e.target.value})} />
                    </td>
                    <td className="p-4 md:p-5 text-center align-middle bg-blue-100/50 md:border-l block md:table-cell">
                      <button onClick={() => salvarEdicao(item.id)} className="w-full bg-green-600 text-white py-2 rounded font-bold text-xs mb-2 shadow">SALVAR</button>
                  ...
                  <>
                    <td className="p-4 md:p-5 align-top md:border-r border-slate-100 block md:table-cell">
                      ...
                      <div className="bg-blue-50 p-2 rounded border border-blue-100">
                        ...
                        <p className="text-sm font-bold text-blue-800 uppercase">{item.convenio || 'Particular'}</p>
                      </div>
                      <div className={`mt-2 text-[9px] font-black uppercase p-1 px-2 rounded-full inline-block ${
                        ...
                        item.status === 'Insucesso' ? 'bg-red-600 text-white' : 'bg-orange-400 text-white'
                      }`}>
                        {item.status}
                      </div>
                    </td>
                    ...
                    <td className="p-4 md:p-5 align-top block md:table-cell">
                      ...
                      {/* EXIBIÇÃO DO CAMPO OBSERVAÇÕES */}
                      {item.observacoes && (
                        <div className="bg-slate-50 border-l-4 border-slate-400 p-3 mb-3">
                          <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">💬 Observações:</p>
                          <p className="text-sm text-slate-700 italic">{item.observacoes}</p>
                        </div>
                      )}

                      <div className="flex items-center gap-6">
                        <div><p className="text-[9px] font-bold text-slate-400 uppercase">Valor:</p><p className="text-[#006363] font-black text-lg">R$ {item.valor}</p></div>
                        <div><p className="text-[9px] font-bold text-slate-400 uppercase">Pagamento:</p><div className="bg-slate-800 text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider inline-block">{item.pagamento}</div></div>
                      </div>
                      
                      <div className="mt-4 pt-2 border-t border-slate-100 text-[11px] text-slate-400 italic">
                        <span>📝 {item.criado_por}</span>
                        {item.atualizado_por && (<span> | 🔄 {item.atualizado_por}</span>)}
                      </div>
                    </td>
                        ...
                        {item.status === 'Pendente' ? (
                          justificandoId === item.id ? (
                            ...
                            <div className="space-y-2">
                              <button onClick={() => finalizarColeta(item.id)} className="w-full bg-[#006363] text-white py-3 rounded-lg font-black text-xs hover:bg-teal-800 shadow">CONCLUIR COLETA ✅</button>
                              <button onClick={() => setJustificandoId(item.id)} className="w-full bg-orange-500 text-white py-2 rounded-lg font-bold text-[10px] shadow">NÃO REALIZADA ❌</button>
                          ...
                          <div className="py-4">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Finalizada</p>
                            <div className="text-2xl mt-2 opacity-50">🔒</div>
                            <div className="flex flex-col gap-2 mt-4">
                                <button onClick={() => iniciarEdicao(item)} className="text-[9px] bg-blue-100 text-blue-600 px-2 py-1 rounded font-bold uppercase">Corrigir</button>
                                <button onClick={() => deletarAgendamento(item.id)} className="text-[9px] text-red-400 hover:underline uppercase font-bold">Excluir</button>
                            </div>
                          </div>
                        )}
                    </td>
